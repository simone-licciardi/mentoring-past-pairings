...
void add_mentor(FILE* mentor_db, char* name, char* surname, int id);
void add_mentee(FILE* mentee_db, char* name, char* surname, int id);
void add_pair(FILE* pairs_db, int id_mentor, int id_mentee, int year);

int main(){
	// declaration
	FILE *mentor_db, *mentee_db, *pairs_db, *new_db;
	char ins[IST_LEN], *entry;
	char mentor_nm[NAME_FIELDS], mentor_snm[NAME_FIELDS];
	char mentee_nm[NAME_FIELDS], mentee_snm[NAME_FIELDS];
	...
	while(fgets(ins,IST_LEN,new_db) != NULL){
		...
		mentor_id = search_mentor(mentor_db,mentor_nm,mentor_snm,&found);
		if(!found) add_mentor(mentor_db,mentor_nm,mentor_snm,mentor_id);

		mentee_id = search_mentee(mentee_db,mentee_nm,mentee_snm,&found);
		if(!found) add_mentee(mentee_db,mentee_nm,mentee_snm,mentee_id);

		add_pair(pairs_db, mentor_id, mentee_id, year);		
	}
...
void add_mentor(FILE* mentor_db, char* name, char* surname, int id){
	fprintf(mentor_db,"%d,%s,%s\n",id,name,surname);
}
void add_mentee(FILE* mentee_db, char* name, char* surname, int id){
	fprintf(mentee_db,"%d,%s,%s\n",id,name,surname);
}
void add_pair(FILE* pairs_db, int id_mentor, int id_mentee, int year){
